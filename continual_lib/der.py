...
class Model(continual_lib.BaseContinualLearner):
    ...
    def __init__(self, args, backbone, head, loss, device, amp_dtype, experiment, alpha, **kwargs):
        super(Model, self).__init__(args, backbone, head, loss, device, amp_dtype)

        assert_str = "Used buffer.size = 0 for buffer-based continual.method=[der]!"
        ...
        if self.training_mode == "classification_task":
            termcolor.cprint(
                "Warning: continual.method=[der] and training=[classification_task] technically work, but will have limited benefits",
                "yellow",
                attrs=[],
            )
    ...
    def observe(self, images, targets, **kwargs):
        ...
        with torch.amp.autocast(self.device.type, self.amp_dtype):
            ...
            if not self.buffer.is_empty():
                ...
                # Note: Logits when adapting with openclip need to be scaled more, since image and text features are normalized.
                # Consequently, the MSE loss has much lower magnitude (in practice ~10-50x).
